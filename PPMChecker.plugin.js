...
function PPMChecker(meta) {
    ...
    let _processedAutoKickMessages = new Set(); // Track processed auto-kick messages to prevent duplicates
    ...
    const parseFullPPMResponse = (text) => {
        ...
        while ((match = userPattern.exec(text)) !== null) {
            ...
            result.users.push({ userId, isLeader, ppm });
        }

        return result.users.length > 0 ? result : null;
    };

    // Fetch display name from Discord UserStore
    const getUserDisplayName = (userId) => {
        if (!_userStore) return null;

        try {
            const user = _userStore.getUser(userId);
            if (user) {
                // Return username (display name) or global_name if available
                return user.globalName || user.username || null;
            }
        } catch (error) {
            log(`Error fetching user ${userId}: ${error.message}`, "warn");
        }
    ...
    const detectAutoKick = (message) => {
        ...
        // Skip if we've already processed this message (prevents duplicate triggers from MESSAGE_UPDATE)
        if (message.id && _processedAutoKickMessages.has(message.id)) {
            return;
        }
        ...
        const searchForAutoKick = (title, description) => {
            ...
            if (hasAutoKickTitle && hasKickMessage) {
                ...
                if (message.id) {
                    _processedAutoKickMessages.add(message.id);
                }
    ...
    const loadDispatcherPatch = async () => {
        try {
            // Method 1: Try getting FluxDispatcher from Stores
            let dispatchModule = BdApi.Webpack.Stores?.FluxDispatcher;
            ...
            // Method 3: Try getByKeys with dispatch and subscribe
            if (!dispatchModule || typeof dispatchModule.dispatch !== 'function') {
                dispatchModule = BdApi.Webpack.getByKeys("dispatch", "subscribe", "register", { searchExports: true });
            }

            // Method 4: Try legacy pattern with _events
            if (!dispatchModule || typeof dispatchModule.dispatch !== 'function') {
                let mod = BdApi.Webpack.getModule(m => m?.dispatch && m?._events, { searchExports: true });
                if (mod) dispatchModule = mod.dispatch ? mod : (mod.default ? mod.default : mod);
            }

            // Method 5: Try waitForModule as last resort
            if (!dispatchModule || typeof dispatchModule.dispatch !== 'function') {
                dispatchModule = await BdApi.Webpack.waitForModule(m => m?._actionHandlers && m?.dispatch);
            }

            if (!dispatchModule || typeof dispatchModule.dispatch !== 'function') throw new Error("No Dispatcher found.");
    ...
    const handleGroupPPMCheck = async (fullData) => {
        ...
        // Determine channel for /close_group command and user notifications
        const userNotificationChannelId = settings.userNotificationChannelId || CONFIG.ENGLISH_CHANNEL_ID || settings.notificationChannelId;
        ...
        if (allUsersZero && fullData.groupId && !settings.forceIndividualStops) {
            ...
            // Send formatted report to user notification channel (small text format like TheHelperHelper)
            const groupName = fullData.groupName || 'unknown';
            const lines = [];

            lines.push(`-# ───────────────────────────────────`);
            lines.push(`-# 📋 **Report for Group:** ${fullData.groupId} (${groupName})`);
            lines.push(``);
            lines.push(`-# __Members__`);

            fullData.users.forEach(user => {
                const leaderBadge = user.isLeader ? ' 👑' : '';
                const displayName = getUserDisplayName(user.userId) || user.userId;
                if (user.ppm === 0) {
                    lines.push(`-# 🔴 ${displayName}${leaderBadge} — 🎁 **${user.ppm.toFixed(2)}**`);
                } else {
                    lines.push(`-# ${displayName}${leaderBadge} — 🎁 **${user.ppm.toFixed(2)}**`);
                }
            });
            ...
            lines.push(``);
            lines.push(`-# 💀 **Dead Group detected.** Closing entire group...`);
            lines.push(`-# Please start in 5 minutes, ${userMentions}`);
            lines.push(`-# ───────────────────────────────────`);
        ...
        if (usersWithZeroPPM.length > 0) {
            ...
            lines.push(``);
            lines.push(`-# 🛑 **Auto-stopped cluster** for ${userMentions}`);
            lines.push(`-# You had 0ppm; please start again in 5 minutes.`);
            lines.push(`-# ───────────────────────────────────`);
    ...
    return {
        ...
        stop: () => {
            ...
            // Clear processed auto-kick messages
            _processedAutoKickMessages.clear();
