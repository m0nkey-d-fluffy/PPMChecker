...
function PPMChecker(meta) {
    ...
    const handleGroupPPMCheck = async (fullData) => {
        ...
        if (allUsersZero && fullData.groupId && !settings.forceIndividualStops) {
            ...
            report += `\n💀 Dead Group detected. Closing entire group... Please start in 5 minutes, ${userMentions}\n`;
            report += `───────────────────────────────────`;
        ...
        if (usersWithZeroPPM.length > 0) {
            ...
            report += `\n🛑 Auto-stopped cluster for ${userMentions}. You had 0ppm; please start again in 5 minutes.\n`;
            report += `───────────────────────────────────`;
